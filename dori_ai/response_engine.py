...
class ResponseEngine:
    """Multi-layer answer router: site -> math -> exact KB -> web -> learned Transformer."""
    def __init__(self, tok, model):
        ...
        self.smalltalk = {
            "안녕":"안녕! 나는 돌이야 🐶 무엇을 같이 알아볼까?",
            "안녕하세요":"안녕! 나는 돌이야 🐶 질문을 정확하게 도와줄게!",
            "고마워":"천만에! 🐶",
    ...
    @staticmethod
    def _asks_site(u): return any(x in u.lower() for x in ("돌이사이트","사이트 정보","site","doldol","돌돌증권","돌이신문"))
    ...
    def _site_answer(self,u,user_id=None,access_token=None):
        ...
        if self._asks_news(u):
            ...
            if limit<=0:
                return "로그인한 사용자의 돌이신문 열람 권한을 확인하지 못했어. 로그인 상태를 확인해줘. 🐶"
        ...
        if any(x in u.lower() for x in ("내 정보","내 등급","내 레벨","내 코인","내 경험치","my profile","my account")):
            profile=site.profile(user_id)
            if not profile: return "로그인한 사용자 정보를 확인하지 못했어. 로그인 상태를 확인해줘. 🐶"
            return (f"👤 {profile.get('name','사용자')}\n"
                    f"등급: Lv.{profile.get('user_level',0)} | 경험치 레벨: {profile.get('exp_level',0)}\n"
                    f"돌돌코인: {int(profile.get('doldolcoin') or 0):,}\n"
                    f"경험치: {int(profile.get('exp') or 0):,}\n"
                    f"칭호: {profile.get('equipped_title') or '없음'}\n"
                    f"읽을 수 있는 돌이신문: 제{profile.get('read_dori_news') or 0}호까지")

        if any(x in u.lower() for x in ("사이트 정보","돌이사이트 정보","site info","사이트에 뭐가 있어")):
            summary=site.site_summary()
            return f"🌐 돌이사이트 정보\n돌이신문: {summary['news_count']}개 (최신 제{summary['latest_news']}호)\n활성 돌돌증권 종목: {summary['stock_count']}개"

        if self._asks_stock(u):
            rows=site.stock()
            if not rows:return "현재 돌돌증권 데이터를 불러오지 못했어. 잠시 후 다시 시도해줘."
            # Specific ticker/name query gets a focused answer.
            q=u.lower(); specific=next((r for r in rows if str(r.get('name','')).lower() in q or str(r.get('ticker','')).lower() in q),None)
            if specific:
                x=specific
                return (f"📈 {x.get('name','')} ({x.get('ticker','')})\n"
                        f"현재가: {int(x.get('current_price') or 0):,} 돌돌코인\n"
                        f"변동: {x.get('change_amount',0):+,} ({x.get('change_percent',0)}%)\n"
                        f"위험도: {x.get('risk_label','미지정')}\n"
                        f"설명: {x.get('description','')}\n"
                        f"특징: {x.get('characteristics','')}")
            out=["📈 현재 돌돌증권 데이터"]
            for x in rows:
                try: price=f"{int(x.get('current_price') or 0):,}"
                except Exception: price=str(x.get('current_price') or '-')
                out.append(f"• {x.get('name','')} ({x.get('ticker','')}): {price} 돌돌코인 | {x.get('change_amount',0):+,} ({x.get('change_percent',0)}%) | 위험도 {x.get('risk_label','미지정')}")
            return "\n".join(out)
        return None
    ...
    def reply(self,user,user_id=None,access_token=None):
        ...
        if ans is None: ans=self.kb.answer(u,threshold=.68)
        ...
        if ans is None:
            ...
            temp=.42 if lang in ("ko","en","ja","zh") else .50
            ans=generate(prompt,self.tok,self.model,120,temp,16,.88)
        ans=clean(ans)
        if bad(ans):
            fallbacks={
                "en":"I don't have enough verified information to answer that reliably. Please make the question more specific.",
                "ja":"十分に確認できる情報がありません。質問をもう少し具体的にしてください。",
                "zh":"我没有足够的可靠信息来回答。请把问题说得更具体一些。",
                "es":"No tengo información verificada suficiente. Intenta hacer la pregunta más específica.",
                "fr":"Je n’ai pas assez d’informations vérifiées. Peux-tu préciser ta question ?",
            }
