...
class ResponseEngine:
    def __init__(self,tok,model):
        ...
        self.smalltalk={"안녕":"안녕! 나는 돌이야 🐶 오늘은 무슨 이야기를 해볼까?","안녕하세요":"안녕! 나는 돌이야 🐶 무엇을 도와줄까?","고마워":"천만에! 도움이 되었다면 기뻐! 🐶","hello":"Hello! I'm Dori AI. 🐶","hi":"Hi! I'm Dori AI. 🐶","hola":"¡Hola! Soy Dori AI. 🐶","bonjour":"Bonjour ! Je suis Dori AI. 🐶","こんにちは":"こんにちは！Dori AIだよ。🐶","你好":"你好！我是Dori AI。🐶","안녕!":"안녕! 나는 돌이야 🐶"}
    ...
    def _site_answer(self,u,user_id=None):
        ...
        if n is not None:
            ...
            if self._asks_quiz(u) or any(k in u for k in ("문제","question","quiz")):
                ...
                # Never reveal the answer merely because a quiz was requested.
    ...
    def reply(self,user,user_id=None):
        ...
        if key in self.smalltalk:ans=self.smalltalk[key]
            ...
            if ans is None:
                ...
                ans=generate(prompt,self.tok,self.model,120,.62,16,.90)
            if self._bad(ans):
