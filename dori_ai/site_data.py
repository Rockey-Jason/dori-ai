"""Read-only bridge to Dori site's Supabase data."""
...
class SiteData:
    ...
    def with_token(self, token):
        return SiteData(token or self.access_token)
    ...
    def _get(self, table, params):
        ...
        req = urllib.request.Request(
            ...
            headers={"apikey": self.key, "Authorization": f"Bearer {bearer}", "Accept": "application/json"},
        )
        try:
    ...
    def is_admin(self, user_id=None):
        ...
        return bool(rows[0].get("is_admin")) and int(rows[0].get("user_level") or 0) >= 10
    ...
    def public_news_all(self):
        return self._get("rockey_news",{"select":"news_number,rockey_news,question,question_type,choice1,choice2,choice3,choice4,choice5","order":"news_number.asc"}) or []

    def public_news(self, number):
        try: n=int(number)
        except Exception: return None
        rows=self._get("rockey_news",{"select":"news_number,rockey_news,question,question_type,choice1,choice2,choice3,choice4,choice5","news_number":f"eq.{n}","limit":"1"})
    ...
    def stock(self, query=None):
        rows=self._get("dori_stocks",{"select":"ticker,name,description,risk_label,current_price,previous_price,change_amount,change_percent,is_active,characteristics,available_shares","is_active":"eq.true","order":"id.asc"}) or []
    ...
    def profile(self, user_id=None):
        ...
        rows=self._get("users", {"select":"name,user_level,exp_level,doldolcoin,exp,equipped_title,read_dori_news", "user_id":f"eq.{user_id}","limit":"1"})
        return rows[0] if rows else None

    def site_summary(self):
        news=self.public_news_all()
        stocks=self.stock()
