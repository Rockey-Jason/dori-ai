...
class SiteData:
    ...
    def readable_news_limit(self, user_id=None):
        ...
        if not rows:
            return 0
        try:
            return max(0, int(rows[0].get("read_dori_news") or 0))
        except Exception:
            return 0
    ...
    def news(self, number, user_id=None):
        ...
        if n < 1 or n > limit:
            return {"denied": True, "number": n, "max_readable": limit}
        rows = self._get("rockey_news", {
    ...
    def stock(self, query=None):
        params = {
            "select": "ticker,name,description,risk_label,current_price,previous_price,change_amount,change_percent,is_active,characteristics,available_shares",
